  ...
  )
{
  ...
  // The AP starts with the MMU disabled and must observe the launch data
  // written by the BSP before PSCI_CPU_ON is called.
  ...
  )
{
  ...
  CopyMem (
    ProcessorInfoBuffer,
    &mCpuMpData.CpuData[ProcessorNumber].Info,
    sizeof (EFI_PROCESSOR_INFORMATION)
    );

  if (HealthData != NULL) {
    HealthData->Uint32 = 0;
    if ((ProcessorInfoBuffer->StatusFlag & PROCESSOR_HEALTH_STATUS_BIT) != 0) {
      HealthData->Bits.Tested = 1;
    }
  }
  ...
  )
{
  ...
  if (mCpuMpData.AllWaitEvent != NULL) {
  ...
  if (FailedCpuList != NULL) {
    ...
    mCpuMpData.FailedCpuList = FailedCpuList;
  ...
  )
{
  ...
  if (mCpuMpData.FailedListIndex == 0) {
    if (mCpuMpData.FailedList != NULL) {
      if (mCpuMpData.FailedCpuList != NULL) {
        *mCpuMpData.FailedCpuList = NULL;
      }
...

  {
    ...
    WaitEvent                = mCpuMpData.AllWaitEvent;
    mCpuMpData.AllWaitEvent  = NULL;
    mCpuMpData.FailedList    = NULL;
    mCpuMpData.FailedCpuList = NULL;

    Status = gBS->SignalEvent (WaitEvent);
  }
  ...
  )
{
  ...
  WriteBackDataCacheRange (&gTcr, sizeof (gTcr));
  WriteBackDataCacheRange (&gTtbr0, sizeof (gTtbr0));
  WriteBackDataCacheRange (&gMair, sizeof (gMair));
...
Error:
  for (Index = 0; Index < mCpuMpData.NumberOfProcessors; Index++) {
    if (mCpuMpData.CpuData[Index].CheckThisAPEvent != NULL) {
      gBS->CloseEvent (mCpuMpData.CpuData[Index].CheckThisAPEvent);
      mCpuMpData.CpuData[Index].CheckThisAPEvent = NULL;
    }
  }

  if (mCpuMpData.CheckAllAPsEvent != NULL) {
    gBS->CloseEvent (mCpuMpData.CheckAllAPsEvent);
    mCpuMpData.CheckAllAPsEvent = NULL;
  }

  if (gApStacksBase != NULL) {
    FreePages (
      gApStacksBase,
      EFI_SIZE_TO_PAGES (mCpuMpData.NumberOfProcessors * gApStackSize)
      );
    gApStacksBase = NULL;
  }

  if (gProcessorIDs != NULL) {
    FreePool (gProcessorIDs);
    gProcessorIDs = NULL;
  }

  if (mCpuMpData.CpuData != NULL) {
    FreePool (mCpuMpData.CpuData);
    mCpuMpData.CpuData = NULL;
  }

  return Status;
}

/** Initialize the ARM MP Services protocol implementation from the MP HOB.

  @retval EFI_SUCCESS           The protocol was initialized.
  @retval EFI_OUT_OF_RESOURCES  Resources could not be allocated.
**/
EFI_STATUS
EFIAPI
MpInitLibInitialize (
  ...
  )
{
  ...
  CoreInfo           = GET_GUID_HOB_DATA (Hob);
  NumberOfProcessors = GET_GUID_HOB_DATA_SIZE (Hob) / sizeof (ARM_CORE_INFO);
  if (NumberOfProcessors < 2) {
    return EFI_NOT_FOUND;
  }

  Status = MpServicesInitializeData (NumberOfProcessors, CoreInfo);
  return Status;
}
  ...
  )
{
  ...
  MpInitLibWhoAmI (&ProcessorIndex);
  ...
  )
{
  ...
  for (Index = 0; Index < mCpuMpData.NumberOfProcessors; Index++) {
    ...
    if (GetApState (CpuData) == CpuStateReady) {
      ...
      // In single-threaded mode only the first ready AP is dispatched here;
      // subsequent APs are started by UpdateApStatus after its predecessor
      // completes.
      if (!SingleThread || (mCpuMpData.FinishCount == 0)) {
